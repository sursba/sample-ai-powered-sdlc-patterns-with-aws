/*
 * ============================================================================
 * WARNING: DOCUMENTATION ONLY - DO NOT USE FOR DEPLOYMENT
 * ============================================================================
 * 
 * This Terraform configuration is for documentation purposes only.
 * It reflects the current state of AWS infrastructure deployed via AWS CLI.
 * 
 * DO NOT RUN: terraform plan, terraform apply, or terraform destroy
 * 
 * For deployments, use AWS CLI commands as specified in deployment-workflow.md
 * ============================================================================
 */

variable "project_name" {
  description = "Name of the project (documentation only)"
  type        = string
  default     = "ai-assistant"
}

variable "environment" {
  description = "Environment name (documentation only)"
  type        = string
  default     = "dev"
}

variable "documents_bucket_name" {
  description = "Actual documents bucket name in AWS"
  type        = string
  default     = "ai-assistant-dev-documents-993738bb"
}

variable "frontend_bucket_name" {
  description = "Actual frontend bucket name in AWS"
  type        = string
  default     = "ai-assistant-dev-frontend-e5e9acfe"
}

variable "cloudfront_distribution_arn" {
  description = "Actual CloudFront distribution ARN for bucket policy"
  type        = string
  default     = "arn:aws:cloudfront::254539707041:distribution/EL8L41G6CQJCD"
}

variable "cloudfront_domain" {
  description = "Actual CloudFront domain name"
  type        = string
  default     = "dq9tlzfsf1veq.cloudfront.net"
}

variable "api_gateway_domain" {
  description = "Actual API Gateway domain for CORS"
  type        = string
  default     = "jpt8wzkowd.execute-api.us-west-2.amazonaws.com"
}

variable "tags" {
  description = "Common tags for S3 resources (documentation only)"
  type        = map(string)
  default = {
    Project     = "ai-assistant"
    Environment = "dev"
    ManagedBy   = "aws-cli"
    Purpose     = "documentation-only"
  }
}