#!/usr/bin/env python3
"""
Lambda Handler for Jenkins MCP Server
...
def lambda_handler(event, context):
    ...
    try:
        ...
        sys.path.append('.')
        from lambda_mcp_server import JenkinsMCPServer
...
def handle_health_check():
    ...
    try:
        ...
        health_result = loop.run_until_complete(jenkins_client.test_connection())
        loop.close()
        ...
        return {
            ...
            'body': json.dumps({
                'status': 'healthy',
                'message': 'Fixed Jenkins MCP Server is running',
...
def handle_protected_resource_metadata():
    ...
    return {
        ...
        'body': json.dumps({
            'resource_server': 'https://example-mcp-api.execute-api.region.amazonaws.com/dev',
            'authorization_servers': ['https://example-oauth-api.execute-api.region.amazonaws.com/dev'],
            'scopes_supported': ['mcp:read', 'mcp:write'],
            'bearer_methods_supported': ['header'],
            'resource_documentation': 'https://github.com/example-org/jenkins-mcp-server'
        })
    }
