...
rm -f deployment/oauth-deployment.zip
...
zip -r ../oauth-deployment.zip . -q
cd ../..

# Check package size
PACKAGE_SIZE=$(du -sh deployment/oauth-deployment.zip | cut -f1)
echo "✅ OAuth Server package created: deployment/oauth-deployment.zip"
